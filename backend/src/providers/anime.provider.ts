...
import { cacheAnimes } from "./cache.provider";
...
export async function searchAnime(params: WithPagination<AnimeSearchParams>) {
    const result = await searchAnimeInternal(params);

    const data = map(result.data, next => next.node);
...
async function searchAnimeInternal(params: WithPagination<AnimeSearchParams>) {
